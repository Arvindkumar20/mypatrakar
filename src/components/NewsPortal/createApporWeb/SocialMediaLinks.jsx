...
const SocialMediaLinks = ({
  ...
  const { webPreview, updateWebPreview } = useContext(PreViewContext);
  const [links, setLinks] = useState({
    fb_link: "https://facebook.com/GlobalNewsNetwork",
    insta_link: "https://instagram.com/globalnews.in",
    twitter_link: "https://x.com/GNNofficial",
    linkedin_link: "https://linkedin.com/company/globalnewsnetwork",
    youtube_link: "https://youtube.com/@GlobalNewsNetwork",
    whats_link: "https://wa.me/919876543210",
    koo_link: "",
    telegram_link: "https://t.me/globalnewsupdates",
  });
  ...
  useEffect(() => {
    ...
    updateWebPreview({...webPreview, socialMedia: formattedLinks});
    setUserRequest((prev) => ({ ...prev, socialMediaLinks: formattedLinks }));
  }, [links, setUserRequest, webPreview, updateWebPreview]);
  ...
  // Social media fields config with icons and styling from HTML
  const socialMediaFields = [
    {
      label: "Facebook",
      name: "fb_link",
    ...
    },
    {
      ...
      name: "koo_link",
      icon: (
        <svg className="w-5 h-5 text-black" fill="none" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm3.62 12.28c-.33.33-.77.5-1.22.5-.46 0-.89-.17-1.22-.5l-1.18-1.18c-.33-.33-.5-.77-.5-1.22s.17-.89.5-1.22l2.39-2.39c.33-.33.77-.5 1.22-.5s.89.17 1.22.5.5.77.5 1.22-.17.89-.5 1.22l-1.2 1.18 1.18 1.18c.33.33.5.77.5 1.22s-.17.89-.5 1.22zm-6.03-3.66c-.33.33-.77.5-1.22.5s-.89-.17-1.22-.5-.5-.77-.5-1.22.17-.89.5-1.22l2.39-2.39c.33-.33.77-.5 1.22-.5s.89.17 1.22.5.5.77.5 1.22-.17.89-.5 1.22l-2.39 2.39z" fill="black"/>
        </svg>
      ),
      bgColor: "bg-[#FFC107]",
  ...
  return (
    <div className="bg-white bg-bacd-dark rounded-lg shadow-md p-6 font-sans">
      ...
      <div className="flex flex-col gap-1 pb-6 border-b border-[#E0E0E0] border700">
        <p className="text-[#1C1C1C]  text-2xl font-bold leading-tight">Social Media Links</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3 pt-6">
        {socialMediaFields.map((field) => {
          ...
          const inputValue = field.name === "whats_link" 
            ? links[field.name] 
            : links[field.name]?.replace(/^https?:\/\/(www\.)?/, "") || "";
