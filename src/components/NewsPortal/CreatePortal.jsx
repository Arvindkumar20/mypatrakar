...
import { useDomainCheck } from "./createPortal/hooks/useDomainCheck";
...
export default function CreatePortal() {
  ...
  // Custom Hooks
  const { packages, isLoadingPackages } = usePackageLoader(formData.region);
  ...
  const { checkDomainAvailability } = useDomainCheck(setDomainResult);
  ...
  const handleChange = (e) => {
    ...
    setFormData((prev) => {
      ...
      if (name === "app_name") {
        updated = {
          ...updated,
          app_package_name: generatePackageName(value),
        };
      }
